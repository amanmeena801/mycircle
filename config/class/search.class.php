<?php
  class search{
    ...
    public function searchInstagram($value){
      ...
      echo "</div></div>";
      echo " <div class='s_d_Mycircles s_d'><span class='s_header'>Mycircles</span><div class='s_d_peo'>";

      $tag = preg_replace("#[\#]#", "", $value);
      $q3 = $this->db->prepare("SELECT DISTINCT Mycircle FROM Mycircle WHERE Mycircle LIKE :Mycircle ORDER BY Mycircle_id LIMIT 12");
      $q3->execute(array(":Mycircle" => "%#$tag%"));
      if ($q3->rowCount() > 0) {
        while ($r3 = $q3->fetch(PDO::FETCH_OBJ)) {
          $hashname = $r3->Mycircle;

          echo
            "<a class='s_d_p h_d_p' href='{$this->DIR}/Mycircle?tag=". substr($hashname, 1) ."'><span class='s_d_username'>". $universal->nameShortener($hashname, 30) ."</span>";
            // echo "<span class='s_d_light'>";
            // if ("#".$tag == $hashname) {
            //   echo $Mycircle->noOfMycircles("$tag")." posts";
            // } else {
            //   echo "Working..";
            // }
            // echo "</span>
