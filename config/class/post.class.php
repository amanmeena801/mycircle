<?php
  class post{
    ...
    public function textPost($value, $tags, $font, $loc, $when, $grp){
      ...
      $Mycircle->getMycircles($value, $last_id);
    ...
    public function addMoreLink($text, $class){
      if(strlen($text) > 1000){
        return "<a href='#' class='Mycircle {$class}'>.. Load more</a>";
      }
    }
    ...
    public function getDifferentPost($type, $post_id, $size){
      ...
      if ($type == "text") {
        ...
        while ($trow = $tquery->fetch(PDO::FETCH_OBJ)) {
          ...
          $text = $Mycircle->toMycircle($text, "post");
          // $text = $Mycircle->lineBreakMycircle($text, "post");
    ...
    public function getUserPost($id, $way, $count){
      ...
      if ($count == 0) {
        ...
        while ($prow = $pquery->fetch(PDO::FETCH_OBJ)) {
          ...
          <span class='p_edit_tip'><i class='fa fa-info-circle' aria-hidden='true'></i>For Mycircle, first remove all the text</span>
    ...
    public function getTaggedPost($id, $way, $count){
      ...
      if ($query->rowCount() == 0) {
        ...
        while ($row = $query->fetch(PDO::FETCH_OBJ)) {
          ...
          echo "<li><a href='#' data-link='{$universal->urlChecker($this->DIR)}/view_post/{$post_id}' class='p_copy_link'>Copy link</a></li>";
          echo "</ul></div></div><div class='p_o'>";
          echo "<div class='p_edit_tools'><span class='p_edit_tip'><i class='fa fa-info-circle' aria-hidden='true'></i>For Mycircle, first remove all the text</span>
          <a href='#' class='p_edit_cancel sec_btn'>Cancel</a>
          <a href='#' class='p_edit_save pri_btn'>Save</a></div>";
    ...
    public function deletePost($post){
      ...
      $hashquery = $this->db->prepare("DELETE FROM Mycircle WHERE post_id = :post");
    ...
    public function editPost($text, $post, $type){
      ...
      $a = $this->db->prepare("DELETE FROM Mycircle WHERE post_id = :post");
      $a->execute(array(":post" => $post));

      $mention->getMentions($text, $post);
      $Mycircle->getMycircles($text, $post);
