<?php
  class Mycircle{
    ...
    public function toMycircle($str, $of){
      $regex = "/#+([^ <>]+)/";
      $two = substr($str, 1);
      $t = preg_replace("#[\#]#", "", $two);
      ...
      if ($t != "") {
        if ($of == "post") {
          $str = preg_replace($regex, '<a class="Mycircle" href="'. $this->DIR .'/Mycircle?tag=$1">$0</a>', $str);
        } else if ($of == "comment") {
          $str = preg_replace($regex, "<span class='Mycircle'>$0</span>", $str);
        }
      }
    ...
    public function lineBreakMycircle($str, $of){
      $newString = '';
      $array = preg_split('/(\r\n|\r|\n)/', $str);
      ...
      foreach ($array as $line) {
        ...
        foreach ($word as $each) {
          ...
          if($each[0] == "#"){
            ...
            if($t != ""){
              if($of == "post"){
                $newString .= "<a class='Mycircle' href='{$this->DIR}/Mycircle?tag={$t}'>{$each}</a> ";
              } else if($of == "comment") {
                $newString .= "<span class='Mycircle'>{$each}</span>";
              }
            }
    ...
    public function getMycircles($text, $post){
      $session = $_SESSION['id'];
      ...
      foreach ($array as $value) {
        if ($value[0] == "#") {
          ...
          if ($t != "") {
            // $mquery = $this->db->prepare("SELECT Mycircle_id FROM Mycircle WHERE post_id = :post AND Mycircle = :Mycircle AND user_id = :user AND src = :src");
            // $mquery->execute(array(":post" => $post, ":Mycircle" => $value, ":user" => $session, ":src" => "post"));
            // if ($mquery->rowCount() == 0) {
            $query = $this->db->prepare("INSERT INTO Mycircle(Mycircle, src, post_id, user_id, time) VALUES (:Mycircle, :src, :post, :user, now())");
            $query->execute(array(":Mycircle" => "#$t", ":src" => "post", ":post" => $post, ":user" => $session));
            // }
          }
        }
      }
    ...
    public function noOfMycircles($hash){
      $query = $this->db->prepare("SELECT Mycircle_id FROM Mycircle WHERE Mycircle LIKE :hash");
    ...
    public function usersMycircles($user){
      ...
      $query = $this->db->prepare("SELECT DISTINCT Mycircle FROM Mycircle WHERE user_id = :user ORDER BY Mycircle_id DESC LIMIT 20");
      $query->execute(array(":user" => $user));
      if ($query->rowCount() > 0) {
        echo "<div class='my_Mycircles inst'><div class='header_of_divs'><span class='my_h_header'>";
        if($user == @$session){ echo "Your"; } else { echo $universal->GETsDetails($user, "username")."'s"; }
        echo " recent Mycircles</span></div><div class='my_h_main'>";
        while ($row = $query->fetch(PDO::FETCH_OBJ)) {
          $Mycircle = $row->Mycircle;
          echo "<a href='{$this->DIR}/Mycircle?tag=". substr($Mycircle, 1) ."'>{$Mycircle}</a>";
        }
        echo "</div></div>";
      }
    }

    public function popularMycircles(){
      $session = $_SESSION['id'];
      $universal = new universal;

      $query = $this->db->query("SELECT Mycircle, COUNT(Mycircle) as c FROM Mycircle GROUP BY Mycircle ORDER BY c DESC LIMIT 10");
      if ($query->rowCount() > 0) {
        echo "<div class='my_Mycircles inst'><div class='header_of_divs'><span class='my_h_header'>Popular trends</span></div>
        <div class='my_h_main'>";
        while ($row = $query->fetch(PDO::FETCH_OBJ)) {
          $Mycircle = $row->Mycircle;
          echo "<a href='{$this->DIR}/Mycircle?tag=". substr($Mycircle, 1) ."'>{$Mycircle}</a>";
    ...
    public function noOfMycirclePosts($tag){
      $query = $this->db->prepare("SELECT post.post_id, post.user_id, post.type, post.post_of, post.grp_id, post.time, post.font_size, post.address FROM post, Mycircle WHERE Mycircle.Mycircle = :Mycircle AND Mycircle.post_id = post.post_id ORDER BY post.time DESC");
      $query->execute(array(":Mycircle" => "#$tag"));
      $count = $query->rowCount();
      if ($count == 0) {
        return "No";
      } else {
    ...
    public function MycirclegedPost($tag, $way, $limit){
      $session = $_SESSION['id'];
      ...
      if ($way == "direct") {
        $query = $this->db->prepare("SELECT post.post_id, post.user_id, post.type, post.post_of, post.grp_id, post.time, post.font_size, post.address, Mycircle.Mycircle_id FROM post, Mycircle WHERE Mycircle.Mycircle = :Mycircle AND Mycircle.post_id = post.post_id ORDER BY Mycircle.Mycircle_id DESC LIMIT 10");
        $query->execute(array(":Mycircle" => "#$tag"));

      } else if ($way == "ajax") {
        $start = intval($limit);
        $query = $this->db->prepare("SELECT post.post_id, post.user_id, post.type, post.post_of, post.grp_id, post.time, post.font_size, post.address, Mycircle.Mycircle_id FROM post, Mycircle WHERE Mycircle.Mycircle = :Mycircle AND Mycircle.post_id = post.post_id AND Mycircle.Mycircle_id < :start ORDER BY Mycircle.Mycircle_id DESC LIMIT 5");
        $query->execute(array(":Mycircle" => "#$tag", ":start" => $start));
      }

      $count = $query->rowCount();
      if ($count == 0) {
        if ($way == "direct") {
          echo "<div class='home_last_mssg Mycircle_last_mssg'><img src='{$this->DIR}/images/needs/large.jpg'><span>No tagged posts found</span></div>";
        }
      } else if ($count > 0) {
        while ($row = $query->fetch(PDO::FETCH_OBJ)) {
          $hashid = $row->Mycircle_id;
          $post_id = $row->post_id;
          $user_id = $row->user_id;
          $type = $row->type;
          $time = $row->time;
